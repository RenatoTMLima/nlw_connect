...
class EventsLinkCreator:
    def __init__(self, repo: EventosLinkRepositoryInterface):
        self.__repo = repo
    ...
    def __check_event_link(self, event_id: int, subscriber_id: int) -> None:
        response = self.__repo.select_event_link(event_id, subscriber_id)
        if response:
            raise Exception("Link already exists!")

    def __create_event_link(self, event_id: int, subscriber_id: int) -> str:
        new_link = self.__repo.insert(event_id, subscriber_id)
        return new_link
